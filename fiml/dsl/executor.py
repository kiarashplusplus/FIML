...
class TaskExecutor:
    ...
    async def _fetch_ohlcv(
        self, task: ExecutionTask, context: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
...
class FKDSLExecutor:
    ...
    async def _execute_plan(self, task_id: str, plan: ExecutionPlan) -> None:
        ...
        try:
            ...
            while len(completed) < len(plan.tasks):
                ...
                # Execute in parallel
                tasks = [self.task_executor.execute(task, context) for task in executable]
    ...
    def get_task_status(self, task_id: str) -> Optional[TaskInfo]:
        ...
        return TaskInfo(
            ...
            type=(
                f"dsl_{internal_info.query_type.lower()}"
                if internal_info.query_type
                else "dsl_execution"
            ),
