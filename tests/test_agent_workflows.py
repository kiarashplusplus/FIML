...
class TestDeepEquityAnalysisWorkflow:
    ...
    async def test_workflow_execute_success(self):
        ...
        with patch("fiml.agents.workflows.provider_registry") as mock_registry:
            ...
            mock_provider.fetch_price = AsyncMock(return_value=MagicMock(
                is_valid=True,
                data={
                    "price": 150.25,
                    "change": 2.5,
            ...
            mock_registry.get_providers_for_asset = AsyncMock(
...
class TestCryptoSentimentAnalysisWorkflow:
    ...
    async def test_signal_generation_bullish(self):
        ...
        signals = await workflow._generate_signals(
            sentiment={
                "sentiment": {"score": 70}  # Bullish sentiment
            },
            technicals={
                "indicators": {"rsi": 40}  # Oversold - bullish
            },
            correlations={}
        )
    ...
    async def test_signal_generation_bearish(self):
        ...
        signals = await workflow._generate_signals(
            sentiment={
                "sentiment": {"score": 30}  # Bearish sentiment
            },
            technicals={
                "indicators": {"rsi": 75}  # Overbought - bearish
            },
            correlations={}
        )
...
class TestWorkflowIntegration:
    ...
    async def test_end_to_end_equity_workflow(self):
        ...
        assert result.status in [WorkflowStatus.COMPLETED, WorkflowStatus.FAILED, WorkflowStatus.PARTIAL]
        assert result.execution_time_ms is not None
