...
async def monitor_dashboard():
    ...
    try:
        async with aiohttp.ClientSession() as session, session.ws_connect(uri) as ws:
...
async def monitor_multiple_assets():
    ...
    uri = "http://localhost:8000/dashboard/assets/monitor"
    ...
    try:
        async with aiohttp.ClientSession() as session, session.get(uri, params=params) as resp:
            if resp.status == 200:
                data = await resp.json()
...
async def create_price_alert():
    ...
    try:
        async with aiohttp.ClientSession() as session, session.post(uri, json=alert_config) as resp:
            if resp.status == 200:
                data = await resp.json()
...
async def manage_watchdogs():
    ...
    try:
        async with aiohttp.ClientSession() as session, session.get(f"{base_uri}/watchdogs") as resp:
            if resp.status == 200:
